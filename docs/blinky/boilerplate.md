...
Let's try to compile this and see what happens! If it's not already there, pull up your terminal in VS Code (Terminal > New Terminal).

Make sure you're in the `blinky` directory on your terminal. This won't do what it needs to if you're in the wrong directory! Then, type `make` and hit enter.
...
```
/workspaces/blinky/src/main.cpp: In function 'int main()':
/workspaces/blinky/src/main.cpp/src/main.cpp:5:5: error: 'bn' has not been declared
...
> 
> If you got an error saying no makefile found, you're in the wrong directory. Use `cd` to find your way to the `blinky` directory and try again. Consider closing and re-opening your VS Code so it's exactly in the `blinky` directory, not above or below it.
>
> Also make sure you've got VS Code open in your Dev Container!
{: .error}
...
But once it's finally done running, the last line should say `ROM fixed!`. If you look at the files, you'll see that a `blinky.gba` file has been created!

There's a lot of other files and folders that get created too. If you're curious, take a look in the `build` folder. There's dozens of files in there that get created when you run make! There's also a `blinky.elf`. Feel free to explore on your own for now, we'll talk more about what these files are later
{: .note }
...
### Git refresher
1. Make sure you're in the correct repository directory by running `pwd` (the path should end with `blinky`). If you're not, `cd` to the correct directory.
